...
<template>
  ...
  >
    <router-link :to="`/hotels/${url}/${hotel.id}`" class="flex items-center">
      <!-- Hotel Image -->
      <img
        :src="hotel.image" class="w-full h-52 md:w-48 md:h-auto object-cover transition-transform transform shadow-md border-4 border-gray-200"
        alt="Hotel Image"
      >
    </router-link>
    ...
    <div class="flex-1 p-4">
      ...
      <div class="flex flex-wrap gap-1 mb-1">
        <span
          v-for="tag in hotel.tags" :key="tag" :class="{
      ...
      <p v-if="hotel.tags.includes('Breakfast Included')" class="text-sm text-green-600 mt-1">
        🍳 Breakfast
        Included
      </p>
